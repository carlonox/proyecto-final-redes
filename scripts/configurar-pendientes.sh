#!/bin/bash
# Configurar pendientes del proyecto final
...
echo "=========================================="
sudo virt-customize -a /tmp/srv-ldap-sm.qcow2 --install cups 2>&1 | tail -3
...
sudo virt-customize -a /tmp/srv-ldap-bar.qcow2 --run-command '
mkdir -p /etc/ldap/content
cat > /etc/ldap/content/base.ldif << LDIF
dn: dc=empresa,dc=local
objectClass: top
